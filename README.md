...
zainiraq-go/
├── pkg/
│   └── zain/                    # حزمة الـ SDK الأساسية (صفر اعتمادات خارجية)
│       ├── client.go            # إعداد العميل، الترويسات، إدارة الجلسات، واستئناف التوكن
│       ├── auth.go              # المصادقة، OTP، تسجيل الدخول بكلمة المرور، والتحقق
│       ├── profile.go           # الملف الشخصي الكامل، تحديث البيانات، وإدارة الحساب
│       ├── dashboard.go         # لوحة التحكم، ملخص الرصيد، الحسابات الفرعية، والفوترة
│       ├── recharge.go          # شحن كروت الشحن، تحويل الرصيد، وتمديد الصلاحية
│       ├── services.go          # تفعيل وإلغاء الباقات، عروض ATL/BTL، نظام فليكس، ومشاركة الباقات
│       ├── loyalty.go           # برنامج الولاء والمكافآت، استبدال النقاط، وعروض امتياز
│       ├── payments.go          # بوابة الدفع الإلكتروني، البطاقات المحفوظة، ومحفظة زين كاش
│       ├── support.go           # مركز الدعم، رفع تذاكر الشكاوى مع المرفقات، وتتبع الحل
│       ├── content.go           # محرك استعلامات المحتوى CMS المباشر (Galleon/Products)
│       ├── types.go             # هياكل البيانات والنماذج المكتوبة بشدة (DTOs)
│       └── client_test.go       # حزمة اختبارات الوحدة الشاملة لكافة الوظائف
├── examples/                    # 10 تطبيقات وأمثلة عملية كاملة وقابلة للتشغيل المباشر
│   ├── 01_otp_login/            # تسجيل الدخول عبر OTP وحفظ الجلسة
│   ├── 02_account_and_profile/  # جلب الملف الشخصي والحسابات الفرعية
│   ├── 03_recharge_and_transfer/# شحن الكارت وتحويل الرصيد وتمديد الصلاحية
│   ├── 04_bundles_and_offers/   # تصفح واشتراك الباقات، عروض فليكس، ومشاركة الحصص
...
│   ├── 07_cms_content_queries/  # استعلامات محرك المحتوى CMS لكتالوج العروض والضبط
│   ├── 08_payments_and_zaincash/# بوابات الدفع الإلكتروني، البطاقات، ومحفظة زين كاش
│   ├── 09_bundle_sharing_and_fnf/# إدارة مجموعات مشاركة الباقات العائلية والأرقام المفضلة
│   ├── 10_nearme_and_notifications/# أقرب فروع زين عبر GPS، إشعارات الصندوق، والقصص
│   ├── interactive_cli/         # واجهة سطر أوامر تفاعلية شاملة (Interactive CLI)
│   └── README.md                # دليل تفصيلي لجميع الأمثلة مع أوامر التشغيل
...
### 3. شحن الرصيد وتحويل الأموال وبوابة الدفع (Recharge, Transfer & Payments)
* **شحن كروت الرصيد الورقية**: `RechargeVoucher(ctx, voucherPIN)` لشحن الكروت ذات الـ 16 رقماً مع استلام الرصيد الجديد فورياً.
* **تحويل الرصيد النقدي (P2P)**: `CreditTransfer(ctx, recipient, amount, otp)` لتحويل رصيد مباشر بين مشتركي زين العراق.
* **تمديد صلاحية استقبال وإرسال الخط**: `ExtendValidity(ctx, amount)` بخصم القيمة من الرصيد الأساسي.
...
* **محفظة زين كاش (ZainCash Direct Purchase)**:
  * `CreatePurchaseOrder(ctx, req)`: إنشاء أمر شراء فوري والدفع المباشر عبر ZainCash.
