...
| # | الطريقة | المسار (Endpoint Path) | الدالة المقابلة (Go SDK) | الوصف التفصيلي باللغة العربية |
| :---: | :---: | :--- | :--- | :--- |
...
#### آلية العزل ومنع التداخل عند تطابق المبالغ (Multi-Customer Isolation & Anti-Collision):
* **هل يخلط النظام بين زبونين إذا حوّلا نفس المبلغ في نفس الوقت؟**
  **مستحيل تماماً**؛ لأن العزل يتم عبر 3 طبقات تقنية محكمة:
  1. **عزل رقم هاتف المرسل (`b_number`)**: كشف حساب زين السحابي لا يعتمد على المبلغ فقط، بل يسجل رقم هاتف المرسل الفعلي من مقسم الشبكة. فإذا حول الزبون (أ) `07801111111` مبلغ 5,000 د.ع، وحول الزبون (ب) `07802222222` مبلغ 5,000 د.ع، فكل عملية ترتبط برقم هاتف صاحبها بدقة مطلقة. دالة `VerifyIncomingTransfer(ctx, "07801111111", 5000)` تبحث حصراً عن رقم الزبون (أ) وتتجاهل أي حوالة أخرى حتى وإن كانت بنفس المبلغ.
  2. **العزل الزمني بالثانية (`start_time`)**: كل حوالة مسجلة بتوقيتها الدقيق بالثانية في خوادم زين (`2026-09-12 05:30:15`).
  3. **منع إعادة الاستخدام والاستهلاك المزدوج (Idempotency & Replay Prevention)**: يعتمد النظام مفتاحاً فريداً لكل عملية:
     `Key = Phone + Amount + Timestamp`
     (كما هو مطبق في كود [المثال 14](examples/14_automated_transfer_matching/main.go))، وبمجرد تأكيد طلب الزبون يُوسم المفتاح كـ "مُستخدم" (`processed`) لضمان عدم إعادة استخدام نفس الحوالة القديمة لأي طلب جديد.
