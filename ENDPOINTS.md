...
## 12. إدارة الشريحة والنظام والوقت (SIM Management & System Time)

### 12.1 مزامنة وقت النظام وخوادم زين (Server Time Synchronization - Endpoint 92)
* **المسار**: `GET /api/system/time`
* **دالة Go SDK**: `client.GetServerTime(ctx)`
* **الترويسات**: ترويسات `Skel-*` القياسية (نقطة عامة Public لا تشترط توكن الجلسة `Bearer`).

#### الوصف الفني والغرض التشغيلي:
تتيح هذه النقطة الاستعلام المباشر عن التوقيت الرسمي والمعتمد لخوادم شركة زين العراق المركزية بتوقيت العاصمة بغداد (`Asia/Baghdad` / `UTC+3`).

**أهمية النقطة في التطبيقات والأنظمة البرمجية والبوتات:**
1. **حساب فارق التوقيت وانحراف الساعة (Clock Drift / Skew)**:
   - بمقارنة ساعة السيرفر المحلي أو هاتف المستخدم بساعة خوادم زين، يتم استخراج الفارق الزمني الدقيق بالملي ثانية.
   - يمنع ذلك رفض طلبات رموز التحقق (OTP) أو المعاملات الحساسة التي ترتبط بنوافذ صلاحية زمنية ضيقة (تتراوح بين 60 إلى 120 ثانية) عند وجود اختلاف في توقيت جهاز المستخدم.
2. **ضبط وإدارة انتهاء صلاحية الجلسات (Token Expiry Management)**:
   - تحديد اللحظة الدقيقة لانتهاء صلاحية توكن الوصول `access_token` بناءً على زمن الخادم لا زمن العميل، مما يسمح بتشغيل التجديد التلقائي عبر `refresh_token` في الخلفية قبل أن تواجه الطلبات خطأ `401 Unauthorized`.
3. **أتمتة العروض اليومية المجدولة (Scheduled Daily Offers Automation)**:
   - معرفة اللحظة الدقيقة لدخول اليوم الجديد عند منتصف الليل (`00:00:00`) بتوقيت بغداد لسحب الهدية المجانية اليومية (`ClaimDailyGift`) أو تفعيل العروض الدورية، مما يمنع إرسال طلبات مبكرة قبل بدء اليوم الجديد وتفادي حظر الطلبات.

#### نموذج طلب HTTP:
```http
GET /api/system/time HTTP/1.1
Host: mw-mobile.iq.zain.com
Skel-Platform: Android
Skel-Fix-Version: 6.6.0
Accept: application/json
```

#### استجابة خادم زين الرسمية (JSON Response):
```json
{
  "status": "success",
  "data": {
    "server_time": "2026-09-12T05:39:22+03:00",
    "timestamp": 1789180762,
    "timestamp_ms": 1789180762000,
...
#### نموذج الاستخدام في Go SDK:
```go
serverTime, err := client.GetServerTime(ctx)
if err != nil {
    log.Fatalf("فشل استعلام توقيت الخادم: %v", err)
}

fmt.Printf("توقيت خوادم زين الرسمي: %s\n", serverTime.ServerTime)
fmt.Printf("المنطقة الزمنية: %s (%s)\n", serverTime.Timezone, serverTime.UTCOffset)
fmt.Printf("طابع الوقت Unix: %d ثانية\n", serverTime.Timestamp)
```

---

### 12.2 قائمة الأرقام المفضلة والأصدقاء (Friends & Family)
تتيح إضافة واستعلام وإزالة الأرقام المفضلة على شريحة زين للاستفادة من تخفيضات تعرفة الاتصال:

* **استعلام الأرقام المفضلة**: `GET /api/number/friends-and-family` (`client.GetFriendsAndFamily(ctx)`)
* **إضافة رقم مفضل**: `POST /api/number/friends-and-family/add` (`client.AddFriendsAndFamily(ctx, targetMSISDN)`)
* **إزالة رقم مفضل**: `DELETE /api/number/friends-and-family/remove` (`client.RemoveFriendsAndFamily(ctx, targetMSISDN)`)

**استجابة استعلام الأرقام المفضلة**:
```json
{
  "status": "success",
  "data": {
...
### 12.3 الشريحة الإلكترونية واستبدال الشريحة (eSIM Details & SIM Swap)

* **استعلام بيانات الشريحة الإلكترونية eSIM**:
  * **المسار**: `GET /api/number/esim-details`
  * **دالة Go SDK**: `client.GetESIMDetails(ctx)`
  * **الاستجابة**:
    ```json
    {
      "status": "success",
      "data": {
        ...
        "matching_id": "MATCH-12345",
        "qr_code_data": "LPA:1$smdp.zain.iq$MATCH-12345",
        "activation_code": "ACT-9988",
        "status": "ACTIVE"
      }
    }
    ```

* **طلب استبدال وتفعيل الشريحة الجديدة (SIM Swap)**:
  * **المسار**: `POST /api/number/swap-sim`
  * **دالة Go SDK**: `client.RequestSIMSwap(ctx, "8996409988776655443")`
  * **طلب JSON**:
    ```json
    {
      "msisdn": "9647801234567",
      "iccid": "8996409988776655443"
    }
    ```
